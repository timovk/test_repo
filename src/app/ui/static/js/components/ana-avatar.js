/**
 * Placeholder portrait for FICTIONAL people (analysis pages).  Same visual language as
 * components/avatar.js (initials on a party gradient with a seeded pattern) but with unsigned
 * shifts, so every generated radius / position is non-negative (avatar.js uses `>>` on a
 * uint32 seed, which yields negative circle radii for about half of the keys).
 */
...
export function portrait(name, { color = "#3a475f", key = name, size = 56 } = {}) {
  ...
  const id = `anav${seed.toString(36)}${++seq}`;
  const shapes = [];
  for (let i = 0; i < 4; i++) {
  ...
  return svg(
    ...
    svg(
      ...
      svg(
        "linearGradient",
        { id, x1: "0", y1: "0", x2: "1", y2: "1", gradientTransform: `rotate(${seed % 90})` },
