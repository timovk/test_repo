/**
 * Deterministic placeholder portraits for the live pages (FICTIONAL people have no photos).
 * Same design as components/avatar.js (initials on a party gradient with a seeded geometric
 * pattern) but uses unsigned shifts, so no seed yields a negative circle radius (avatar.js logs
 * "attribute r: A negative value is not valid" for about half of the portrait keys).
 */
...
export function portrait(name, { color = "#3a475f", key = name, size = 56 } = {}) {
  const seed = hash(String(key || name || "x"));
  const id = `lvp${seed.toString(36)}${(uid++).toString(36)}`;
  const angle = seed % 90;
  const shapes = [];
  for (let i = 0; i < 4; i++) {
  ...
  return svg(
    ...
    svg(
      ...
      svg(
        "linearGradient",
        { id, x1: "0", y1: "0", x2: "1", y2: "1", gradientTransform: `rotate(${angle})` },
